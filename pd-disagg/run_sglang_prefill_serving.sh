...
./run_serving_benchmark.sh 2>&1 | tee /run_logs/${SLURM_JOB_ID}/bench-servign-pd-disagg_1P1D.log


## Running accuracy numbers.
#cd /sgl-workspace/sglang/benchmark/mmlu
#bash download_data.sh
#python bench_sglang.py --nsub 10 --base-url http://127.0.0.1:8000 2>&1 | tee /run_logs/${SLURM_JOB_ID}/accuracy_pd_disagg_1P1D.log 
#cat result.jsonl | grep -oP '"accuracy": \K\d+\.\d+'
#./run_accuracy.sh 2>&1 | tee /run_logs/${SLURM_JOB_ID}/accuracy_pd_disagg_1P1D.log

#python3 -m sglang.bench_serving  --model $MODEL_PATH --base-url http://127.0.0.1:8000  --max-concurrency 640  --request-rate 5 --num-prompts 640 --random-input-len 1 --random-output-len 1500 --dataset-path /home/csrikris/work/pd-disagg-inference/docker-scripts/05-19/ShareGPT_V3_unfiltered_cleaned_split.json  --random-range-ratio 1
